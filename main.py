...
def adicionar_item_tarefa(tarefa):
    ...
    botao_editar = tk.Button(frame_tarefa, text="Editar", command=lambda f=frame_tarefa, l=label_tarefa: preparar_edicao(f, l), bg="white", relief=tk.FLAT)
    botao_editar.pack(side=tk.RIGHT, padx=5)

    botao_deletar = tk.Button(frame_tarefa,text='Deletar', command=lambda f=frame_tarefa: deletar_tarefa(f), bg="white", relief=tk.FLAT)
...
botao_adicionar = tk.Button(frame, text="Adicionar Tarefa", command=adicionar_tarefa, bg="#4CAF50", height=1, width=15, font=("Roboto", 11), relief=tk.FLAT)
